...
namespace Problème_scientifique___Image
{
    class Buddhabrot_classique
    {
        ...
        public Buddhabrot_classique(int taille, int iteration, long hitsMax)
        {
            if (taille <= 0)
            {
        ...
        public Buddhabrot_classique(int taille, int iteration, long hitsMax, int IterationRed, int IterationGreen, int IterationBlue)
        {
            if (taille <= 0)
            {
                throw new ArgumentException("Taille doit être plus grand que 0");
        ...
        public void Run()
        {
            ...
            do
            {
                Methods.StatusUpdate(this.PourcentageCompleted, this.stopwatch);
                if (this.IsMonochrome)
                {
                    CreationOrbite();
                }
                else if (!this.IsMonochrome) CreationOrbiteCouleur();
        ...
        private void CreationOrbite()
        {
            ...
            double c_r = rand.NextDouble() * 2 * (Xmax - Xmin) + Xmin;
            double c_i = rand.NextDouble() * 2 * (Ymax - Ymin) + Ymin;
        ...
        public Pixel[,] Draw()
        {
            ...
            double limite = GetNormalizer(this.Hits);

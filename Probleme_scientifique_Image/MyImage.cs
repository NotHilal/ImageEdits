...
namespace Problème_scientifique___Image
{
    public class MyImage
    {
        ...
        public MyImage(string filename)
        {
            ...
            if (!oldPath.Contains("Probleme_scientifique_Test_Unitaire"))
            {
                string newPath = Methods.MoveUp(oldPath, 3);
                newPath += "\\Probleme_scientifique_Image\\bin\\Debug";
            ...
            else
            {
                ...
                if (this.largeur % 4 != 0)
                {   
                    this.octetManquant = 4 - ((largeur * 3) % 4);
                }
                for (int ligne = 0; ligne < this.hauteur; ligne++, decalage += this.octetManquant)
                {
                    for (int colonne = 0; colonne < this.largeur; colonne++, decalage += 3)
                    {
        ...
        public MyImage(string filename, int hauteur, int largeur)
        {
            ...
            if (((largeur * 3) % 4) == 0) this.octetManquant = 0;
            else this.octetManquant = 4 - ((largeur * 3) % 4);
        }
        ...
        public int Octet_manquant
        {
            get { return this.octetManquant; }
        }
        ...
        /// </summary>
        /// <param name="filename">Nom de l'image (sans le .bmp!)</param>
        ...
        /// <summary>
        /// Fonction de débug. Affiche le header d'une image et ainsi que la matrice de pixel
        /// </summary>
        ...
        /// <summary>
        /// Convertir une suite d'octet en endian en un nombre. (integer)
        /// </summary>
        /// <param name="tab">Tableau qui contient les octets en format little endian</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// Convertir un entier en une suite d'octet sous le format endian.
        /// </summary>
        /// <param name="val">Valeur entière qui sera convertie en format endian</param>
        /// <param name="tailleEndian">Paramètre qui détermine la taille du tableau qui contiendra les octets sous le format little endian</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// Fonction qui permet de transformer une matrice pixel en une image .bmp.
        /// <para>Fonction qui tiens en compte du nombre d'octets manquants et pose de manière appropriée</para>
        /// </summary>
        /// <param name="image">Nom final du fichier</param>
        /// <param name="fromScratch">Booléen qui permet de déterminer si l'image est créée à partir de rien (ie. à partir d'aucune instance de MyImage déjà existante).
        /// <para>Utilisé pour les fractales et le QRCode</para></param>
            ...
            try
            {
                ...
                for (int ligne = 0; ligne < this.mat_pixel.GetLength(0); ligne++)
                {
                    ...
                    for (int colonne = 0; colonne < this.mat_pixel.GetLength(1) + this.octetManquant; colonne++)
                    {
                        ...
                        {
                            file_bon_format[decalage + increment] = (byte)this.mat_pixel[ligne, colonne].Red;
                            file_bon_format[decalage + increment + 1] = (byte)this.mat_pixel[ligne, colonne].Green;
                            file_bon_format[decalage + increment + 2] = (byte)this.mat_pixel[ligne, colonne].Blue;
                            if (colonne != this.largeur - 1) increment += 3;
                        }
                        else
                        {
        ...
        /// <summary>
        /// Transforme une image en un nuance de gris et de noir
        /// </summary>
        public void Nuancier_gris_noir()
        {
            ...
            nouvelle_image.From_Image_To_File($"{filename}_nuanceDeGris", false);
        }
        public void NoirBlanc()
        {
            ...
            for (int ligne = 0; ligne < this.mat_pixel.GetLength(0); ligne++)
            {
                ...
                {
                    Pixel pixel;
                    int moyenne = (this.mat_pixel[ligne, colonne].Red + this.mat_pixel[ligne, colonne].Green + this.mat_pixel[ligne, colonne].Blue) / 3;
                    if (moyenne < 150) pixel = new Pixel(0, 0, 0);
                    else pixel = new Pixel(255, 255, 255);
                    new_mat[ligne, colonne] = pixel;
                }
            }
            nouvelle_image.mat_pixel = new_mat;
        ...
        public void Colorisation(string couleur_a_prioriser = "g", string couleur_a_changerString = "r", int seuil = 220)
        {
            ...
            for (int ligne = 0; ligne < this.hauteur; ligne++)
            {
                for (int colonne = 0; colonne < this.largeur; colonne++)
                {
                    ...
                    new_mat[ligne, colonne] = new Pixel(r, g, b);
                }
            }
            nouvelle_image.mat_pixel = new_mat;
        ...
        /// <summary>
        /// Fonction qui retourne une image ayant l'effet miroir appliquée sur l'image originale
        /// </summary>
        public void Miroir()
        {
            MyImage nouvelle_image = new MyImage(this.filename);
        ...
        /// </summary>
        /// <param name="coeff">Coefficient d'aggrandissement, c'est un entier</param>
        /// <param name="naming">Permet de déterminer si on veut nommer avec le coeff à la fin ou non
        /// <para>Utile pour le forms.</para></param>
        public void Agrandir(int coeff, bool naming = false)
        {
            ...
            if (((nouvelle_image.largeur * 3) % 4) == 0) nouvelle_image.octetManquant = 0;
            else nouvelle_image.octetManquant = 4 - ((nouvelle_image.largeur * 3) % 4);
            nouvelle_image.taille_fichier = nouvelle_image.nb_bit * nouvelle_image.hauteur * nouvelle_image.largeur;
            nouvelle_image.file = new byte[nouvelle_image.taille_offset + ((nouvelle_image.largeur * 3) + nouvelle_image.octetManquant) * nouvelle_image.hauteur];
            for (int ligne = 0; ligne < this.mat_pixel.GetLength(0); ligne++)
            {
                for (int colonne = 0; colonne < this.mat_pixel.GetLength(1); colonne++)
                {
            ...
            nouvelle_image.mat_pixel = new_mat;
            if(naming) nouvelle_image.From_Image_To_File($"{filename}_agrandit_coeff", false);
            else nouvelle_image.From_Image_To_File($"{filename}_agrandit_coeff{coeff}", false);
        }
        /// <summary>
        /// Fonction pour rétrecir une image
        /// </summary>
        /// <param name="coeff">Coefficient de rétrécissement, c'est un entier</param>
        public void Retrectir(int coeff)
        {
            ...
            nouvelle_image.hauteur = this.hauteur / coeff;
            nouvelle_image.largeur = this.largeur / coeff;
            nouvelle_image.octetManquant = ((nouvelle_image.Largeur * 3) % 4) == 0 ? 0 : 4 - ((nouvelle_image.Largeur * 3) % 4); ;
            double x_scale = this.largeur / nouvelle_image.largeur;
            double y_scale = this.hauteur / nouvelle_image.hauteur;

            nouvelle_image.file = new byte[nouvelle_image.taille_offset + ((nouvelle_image.largeur * 3) + nouvelle_image.octetManquant) * nouvelle_image.hauteur];
            for (int ligne = 0; ligne < nouvelle_image.hauteur; ligne++)
            {
                for (int colonne = 0; colonne < nouvelle_image.largeur; colonne++)
                {
                    int x_scaled = (int)Math.Floor(ligne * x_scale);
                    int y_scaled = (int)Math.Floor(colonne * y_scale);
                    new_mat[ligne, colonne] = this.mat_pixel[x_scaled, y_scaled];
                }
            }
            nouvelle_image.mat_pixel = new_mat;
            nouvelle_image.From_Image_To_File($"{filename}_retrecissement_coeff{coeff}", false);
        }
        /// <summary>
        /// Fonction pour effectuer une rotation d'un angle quelconque (positif ou négatif) en degré
        /// </summary>
        ...
        public void Rotation(int angle)
        {
            ...
            {
                if (angle < 0) this.Rotation180AntiClockWise();
                else this.Rotation180ClockWise();
            }
        }
        /// <summary>
        /// Rotation d'un angle de 180 degré clockwise
        /// On utilise cette fonction car le shearing ne peut faire une rotation de 180 (ou -180) degrés sans afficher une image erronée
        /// </summary>
        public void Rotation180AntiClockWise()
        {
            ...
            nouvelle_image.mat_pixel = new_mat;
            nouvelle_image.From_Image_To_File($"{this.filename}_rotation{-180}", false);
        }
        /// <summary>
        /// Rotation d'un angle de 180 degré clockwise
        /// On utilise cette fonction car le shearing ne peut faire une rotation de 180 (ou -180) degrés sans afficher une image erronée
        ...
        public void MatriceDeConvolution(double[,] convolution_mat, string nom)
        {
                                                                           ...
                                                                           //faire en sorte que l'application de la matrice de convolution ne déborde pas
            {
                for (int colonne = 1; colonne < new_mat.GetLength(1) - 1; colonne++)
                {
                    ...
                    new_mat[ligne, colonne] = new Pixel(moyenneR, moyenneG, moyenneB);
                }
            }
            nouvelle_image.mat_pixel = new_mat;
        ...
        /// <summary>
        /// Fonction qui retourne l'histogramme des couleurs d'une image
        /// </summary>
        ...
        /// <summary>
        /// Fonction qui permet d'encoder une image dans une autre 
        /// <para>Attention ! On ne peut pas encoder une image plus grande dans une plus petite!</para>
        /// </summary>
            ...
            else
            {
                ...
                for (int ligne = 0; ligne < imageFille.mat_pixel.GetLength(0); ligne++)
                {
                    for (int colonne = 0; colonne < imageFille.mat_pixel.GetLength(1); colonne++)
                    {
                        ...
                        new_mat[ligne, colonne] = new Pixel(octetR, octetG, octetB);
                    }
                }
                nouvelle_image.mat_pixel = new_mat;
        ...
        /// <summary>
        /// Fonction qui permet de décoder l'image encodée
        /// </summary>
        /// <param name="imageMere"></param>
        /// <param name="imageFille"></param>
        public static void DecoderImage(MyImage imageMere, MyImage imageFille)
        {
            ...
            nouvelle_imageFille.taille_fichier = nouvelle_imageFille.nb_bit * nouvelle_imageFille.hauteur * nouvelle_imageFille.largeur;
            nouvelle_imageFille.file = new byte[nouvelle_imageFille.taille_offset + ((nouvelle_imageFille.largeur * 3) + nouvelle_imageFille.octetManquant) * nouvelle_imageFille.hauteur];
            Pixel[,] new_matF = new Pixel[nouvelle_imageFille.hauteur, nouvelle_imageFille.largeur];
            for (int ligne = 0; ligne < imageFille.mat_pixel.GetLength(0); ligne++)
            {
                for (int colonne = 0; colonne < imageFille.mat_pixel.GetLength(1); colonne++)
                {
                    ...
                    new_matF[ligne, colonne] = new Pixel(octetR, octetG, octetB);
                }
            }
        ...
        /// <para>Attention ! Si le choix est "gradientMulti" il faut spécifier les paramètres suivants "pos1", "pos2", "pos3"</para></param>
        ...
        /// <param name="pos1">Détermine la position des couleurs prises par la case rouge dans RGB</param>
        /// <param name="pos2">Détermine la position des couleurs prises par la case vert dans RGB</param>
        /// <param name="pos3">Détermine la position des couleurs prises par la case rouge bleu dans RGB</param>
        /// <param name="time">Paramètre qui permet de donner le fichier avec la classe DateTime</param>
        public static void Fractale(int zoom, int iteration_maximale, string choixFractale = "mandelbrot", string methodeCouleur = "gradientMono", string colorMap = "jet", byte pos1 = 1, byte pos2 = 2, byte pos3 = 3, string time = "")
        {
            #region Variables
            ...
            MyImage nouvelle_image = new MyImage($"fractale_{choixFractale}_{methodeCouleur}_{colorMap}_{pos1}{pos2}{pos3}{time}", hauteur, largeur);
            ...
            for (int ligne = 0; ligne < hauteur; ligne++)
            {
                for (int colonne = 0; colonne < largeur; colonne++)
                {
                    ...
                    else
                    {
                        ...
                        if (methodeCouleur == "HSV") new_mat[ligne, colonne] = new Pixel(R, G, B);
        ...
        #endregion
        /// <summary>
        /// Fonction de test utilisée pour le debugage
        /// </summary>
        public static void CreationImageTest() //Fonction de test qui permet de vérifier l'hypothèse qu'une image avec une taille impaire = marche pas 
        {
            int taille = 21;
            MyImage nouvelle_image = new MyImage("ImageTest", taille, taille);
            int compteur = 0;
            Pixel[,] new_mat = new Pixel[taille, taille];
            for (int ligne = 0; ligne < taille; ligne++)
            {             
                for (int colonne = 0; colonne < taille; colonne++)
                { 
                    if (compteur == 256) compteur = 0;
                    new_mat[ligne, colonne] = new Pixel(compteur, 0, 1);
                    compteur++;
                }
            }
            nouvelle_image.taille_fichier = nouvelle_image.nb_bit * nouvelle_image.hauteur * nouvelle_image.largeur;
            nouvelle_image.file = new byte[nouvelle_image.taille_offset + ((nouvelle_image.largeur * 3) + nouvelle_image.octetManquant) * nouvelle_image.hauteur];
            nouvelle_image.mat_pixel = new_mat;
            nouvelle_image.From_Image_To_File($"ImageTest{taille}", true);
            Affichage_image($"ImageTest{taille}");
        }
        #endregion
    }
}
